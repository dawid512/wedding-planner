...
77. **Goście — przeprojektowanie parowania (grupy, kolory, % cena)** — `partnerId` zastąpiony `groupId?: string`; grupy mogą mieć >2 osoby (para + dziecko itp.); każda grupa ma własny kolor (8-kolorowa paleta, distinct per grupę); nagłówek kolumny "Cena" zamiast "Typ / cena"; etykiety 100%/50%/0% zamiast Dorosły/Dziecko; usunięto dropdown "Strona" z trybu edycji (przypisanie wynika z sekcji tabeli); parowanie możliwe tylko w obrębie tej samej strony; tryb edycji: "↔ Sparuj" → "Wyjdź z grupy" + "Dodaj do grupy".
...
11. ~~**Obsługa zdjęć i grafik**~~ ✅ ZROBIONE — `getAuthToken()` w auth.tsx, `uploadImageToDrive()` / `getDriveThumbnailUrl()` / `deleteDriveFile()` / `findOrCreateSubfolder()` w google-drive.ts; `InspirationItem.photoId?: string`; komponent `PhotoUpload` (drag&drop, upload do `WeddingPlanner/attachments/`, podgląd, usuwanie) w `_shared.tsx`; podpięty do każdej karty moodboardu (inspiration.tsx).
