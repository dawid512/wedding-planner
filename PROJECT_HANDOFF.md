...
11. ~~**Obsługa zdjęć i grafik**~~ ✅ ZROBIONE (2026-05-20) — `getAuthToken()` w `auth.tsx`; `uploadImageToDrive()`, `getDriveThumbnailUrl()`, `deleteDriveFile()`, `findOrCreateSubfolder()` w `google-drive.ts`; `InspirationItem.photoId?: string` w `core.tsx`; komponent `PhotoUpload` w `_shared.tsx` (drag&drop, upload do `WeddingPlanner/attachments/`, podgląd przez `drive.google.com/thumbnail`, usuwanie z Drive); podpięty do kart moodboardu w `inspiration.tsx`. Foldery: `WeddingPlanner/attachments/` na Drive użytkownika.
...
14. ~~**Weryfikacja Google OAuth**~~ ✅ ZROBIONE (2026-05-20) — `VERIFICATION.md` z checklistą, gotowymi tekstami do formularzy Google (EN), uzasadnieniem zakresu `drive` vs `drive.file`, krokami Cloud Console, wymaganiami dla restricted scope (film demo + CASA assessment). Zakres `drive` jest Restricted — wymaga nagrania demo na YouTube + ewentualnego audytu CASA. Przed wysłaniem formularza: uzupełnij OAuth Consent Screen, dodaj logo, zweryfikuj domenę w Search Console, nagraj film demo.
