// Wedding Planner — page components (part 3)

import React from "react";
import { Field as F3, Icon as I3, fmtCurrency as fmt$3, fmtDate as fmtD3 } from "./core";
...
// EVENTS
// ============================================================
function PageEvents({ data, set, editing }: PageProps) {
...
function PageMusic({ data, set, editing }: PageProps) {
  const m = data.music;
...
function PageDocuments({ data, set, editing }: PageProps) {
        ...
        {(["witness1", "witness2"] as WitnessKey[]).map((who) => (
          <div className="card" key={who}>
            ...
            <div className="row" style={{ gridTemplateColumns: "140px 1fr" }}>
              ...
              <F3 value={ddAsRecord[who]?.name} onChange={(v) => updPerson(who, "name", v)} placeholder="—" editing={editing} />
            </div>
            <div className="row" style={{ gridTemplateColumns: "140px 1fr" }}>
              <div className="row__label">Telefon</div>
              <F3 value={ddAsRecord[who]?.phone} onChange={(v) => updPerson(who, "phone", v)} placeholder="+48…" editing={editing} />
            </div>
            <div className="row" style={{ gridTemplateColumns: "140px 1fr", borderBottom: "none" }}>
              <div className="row__label">Nr dokumentu</div>
              <F3 value={ddAsRecord[who]?.id} onChange={(v) => updPerson(who, "id", v)} placeholder="—" editing={editing} />
            </div>
          </div>
        ))}
      </div>
...
// PAYMENTS
// ============================================================
function PagePayments({ data, set, editing }: PageProps) {
