...
type TweakOptionPrimitive = string | number | boolean;
type TweakOptionObj = { value: TweakOptionPrimitive; label: string };
type TweakOption = TweakOptionPrimitive | TweakOptionObj;
type TweakColorOption = string | string[];
type TweakRecord = Record<string, TweakOptionPrimitive | string[]>;

// ── useTweaks ────────────────────────────────────────────────────────────────

function useTweaks<T extends TweakRecord>(defaults: T): [T, (keyOrEdits: string | Partial<T>, val?: TweakOptionPrimitive | string[]) => void] {
  const [values, setValues] = React.useState<T>(defaults);
  const setTweak = React.useCallback((keyOrEdits: string | Partial<T>, val?: TweakOptionPrimitive | string[]) => {
...
function TweaksPanel({ title = "Tweaks", noDeckControls = false, children }: TweaksPanelProps) {
  ...
  const [railEnabled, setRailEnabled] = React.useState<boolean>(
  ...
  const [railVisible, setRailVisible] = React.useState<boolean>(() => {
...
function TweakSlider({ label, value, min = 0, max = 100, step = 1, unit = "", onChange }: TweakSliderProps) {
  return (
...
type TweakRadioProps = {
  label: string;
  value: TweakOptionPrimitive;
  options: TweakOption[];
  onChange: (value: TweakOptionPrimitive) => void;
};

function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  const trackRef = React.useRef<HTMLDivElement | null>(null);
  const [dragging, setDragging] = React.useState(false);
  const valueRef = React.useRef<TweakOptionPrimitive>(value);
  ...
  if (!fitsAsSegments) {
    const resolve = (s: string): TweakOptionPrimitive => {
  ...
  const segAt = (clientX: number): TweakOptionPrimitive => {
...
function TweakText({ label, value, placeholder, onChange }: TweakTextProps) {
  return (
    <TweakRow label={label}>
...
function TweakNumber({ label, value, min, max, step = 1, unit = "", onChange }: TweakNumberProps) {
...
type TweakColorProps = {
  label: string;
  value: TweakColorOption;
  options?: TweakColorOption[];
  onChange: (value: TweakColorOption) => void;
};
...
function TweakColor({ label, value, options, onChange }: TweakColorProps) {
  if (!options || !options.length) {
    ...
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl"><span>{label}</span></div>
        <input type="color" className="twk-swatch" value={singleVal}
               onChange={(e) => onChange(e.target.value)} />
      </div>
    );
  }
